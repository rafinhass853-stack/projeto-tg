...
const GerenciadorDeCargas: React.FC = () => {
  ...
  const styles = {
    ...
    textarea: { width: '100%', height: '120px', padding: '20px', borderRadius: '16px', border: '2px solid #333', fontSize: '15px', marginBottom: '20px', outline: 'none', transition: 'all 0.2s', resize: 'none' as const, backgroundColor: '#1A', color: '#FFF' },
    previewBox: { backgroundColor: '#1A1A1A', borderRadius: '20px', padding: '24px', border: '1px solid #333' },
    ...
    iconBox: { padding: '10px', borderRadius: '12px', backgroundColor: '#1A1A', border: '1px solid #333' },
    ...
    btnSecondary: { backgroundColor: '#1A1A1A', color: '#AAA', padding: '10px 20px', borderRadius: '12px', border: '1px solid #333', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' },
    ...
    filterBar: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginBottom: '24px', backgroundColor: '#0A0A', padding: '24px', borderRadius: '20px', border: '1px solid #1A1A1A' },
    filterGroup: { display: 'flex', flexDirection: 'column' as const, gap: '6px' },
    filterLabel: { fontSize: '12px', fontWeight: 700, color: '#666' },
    filterInput: { padding: '10px 14px', borderRadius: '10px', border: '1px solid #333', fontSize: '14px', outline: 'none', backgroundColor: '#1A', color: '#FFF' },
    filterSelect: { padding: '10px 14px', borderRadius: '10px', border: '1px solid #333', fontSize: '14px', outline: 'none', backgroundColor: '#1A1A1A', color: '#FFF' },
    ...
    itemCard: (status: string) => ({ backgroundColor: '#0A', borderRadius: '24px', padding: '24px', marginBottom: '20px', border: '1px solid #1A1A1A', opacity: status === 'finalizada'? 0.8 : 1, transition: 'all 0.2s' }),
    ...
    statusBadge: (status: string) => ({ padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 700, backgroundColor: status === 'finalizada'? '#22C55E20' : '#FFD70020', color: status === 'finalizada'? '#22C55E' : '#FFD700', display: 'flex', alignItems: 'center', gap: '6px' }),
    logisticsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '24px', marginBottom: '24px' },
    locationBox: (color: string) => ({ padding: '20px', borderRadius: '20px', backgroundColor: `${color}10`, border: `1px solid ${color}30`, position: 'relative' as const }),
    ...
    cityName: { fontSize: '18px', fontWeight: 800, color: '#FFF', margin: '0 0 4px 0' },
    localName: { fontSize: '14px', color: '#888', margin: 0, fontWeight: 500 },
    mapLink: (color: string) => ({ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: color, textDecoration: 'none', fontWeight: 700, marginTop: '12px', padding: '6px 12px', backgroundColor: '#1A1A1A', borderRadius: '8px' }),
    trocaBanner: { padding: '20px', borderRadius: '20px', backgroundColor: '#FFD70020', border: '1px solid #FFD700', display: 'flex', flexDirection: 'column' as const, gap: '8px' },
    vehicleDetails: { padding: '20px', borderRadius: '20px', backgroundColor: '#1A1A', border: '1px solid #333' },
    detailRow: { display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '8px' },
    footer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '20px', borderTop: '1px solid #1F1F1F', flexWrap: 'wrap' as const, gap: '12px' },
    ...
    modalOverlay: { position: 'fixed' as const, top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0.9)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' },
    modalContent: { backgroundColor: '#0A0A0A', borderRadius: '28px', width: '100%', maxWidth: '1100px', maxHeight: '90vh', overflowY: 'auto' as const, border: '1px solid #FFD700', position: 'relative' as const },
    modalHeader: { padding: '24px 32px', borderBottom: '1px solid #1A1A1A', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky' as const, top: 0, backgroundColor: '#0A', zIndex: 10, borderTopLeftRadius: '28px', borderTopRightRadius: '28px' },
    modalBody: { padding: '32px' },
    statCard: { backgroundColor: '#1A1A1A', borderRadius: '20px', padding: '20px', border: '1px solid #333', transition: 'transform 0.2s' },
    statNumber: { fontSize: '32px', fontWeight: 800, color: '#FFF', margin: 0 },
    statLabel: { fontSize: '12px', color: '#666', margin: 0, fontWeight: 500 },
    ...
    formInput: { padding: '12px 16px', borderRadius: '12px', border: '1px solid #333', fontSize: '14px', outline: 'none', backgroundColor: '#1A1A', color: '#FFF' },
    formSectionTitle: { fontSize: '16px', fontWeight: 800, color: '#FFF', margin: '24px 0 16px 0', paddingBottom: '8px', borderBottom: '2px solid #FFD700' },
    groupHeader: { backgroundColor: '#1A1A1A', padding: '12px 24px', borderRadius: '16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px', border: '1px solid #333' },
    groupTitle: { fontSize: '16px', fontWeight: 800, color: '#AAA', margin: 0, textTransform: 'uppercase' as const, letterSpacing: '0.5px' }
  };
    ...
    <div style={styles.container}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        input:focus, select:focus, textarea:focus { border-color: #FFD700!important; box-shadow: 0 3px rgba(255, 215, 0, 0.2)!important; }
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <header style={styles.header} className="no-print">
          <div>
            ...
            <p style={{ margin: '8px 0 0', color: '#666', fontSize: '14px' }}>Gestão Inteligente de Transportes</p>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }} className="no-print">
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <h2 style={{ fontSize: '22px', fontWeight: 800, color: '#FFF', margin: 0 }}>Cargas Programadas</h2>
            <button
              onClick={() => setShowFilters(!showFilters)}
              style={{...styles.actionBtn(showFilters? '#FFD700' : '#1A', showFilters? '#000' : '#AAA'), border: '1px solid #333' }}
            >
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button
              ...
              style={styles.actionBtn(mostrarFinalizadas? '#22C55E' : '#1A1A', mostrarFinalizadas? '#000' : '#AAA')}
            >
                  ...
                  <div key={carga.id} style={styles.itemCard(carga.status)}>
                    <div style={styles.itemHeader}>
                      <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                        <div style={{ background: 'linear-gradient(135deg, #FFD700, #FFA500)', padding: '12px', borderRadius: '16px' }}>
                          <Truck size={24} color="#000" />
                        </div>
                        <div>
                          <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#FFF' }}>{carga.motorista}</h3>
                          <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
                            <span style={{ fontSize: '13px', color: '#888', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <div style={styles.logisticsGrid}>
                      <div style={styles.locationBox('#22C55E')}>
                        ...
                        <p style={{ fontSize: '12px', color: '#666', marginTop: '8px', fontWeight: 600 }}>{carga.coletaData}</p>
                        {carga.coletaLink && (
                          <a href={carga.coletaLink} target="_blank" rel="noreferrer" style={styles.mapLink('#22C55E')}>
                      ...
                      <div style={styles.locationBox('#FFD700')}>
                        ...
                        <p style={{ fontSize: '12px', color: '#666', marginTop: '8px', fontWeight: 600 }}>{carga.entregaData}</p>
                        {carga.entregaLink && (
                          <a href={carga.entregaLink} target="_blank" rel="noreferrer" style={styles.mapLink('#FFD700')}>
                            <MapPin size={14} /> Ver no Mapa <ExternalLink size={12} />
                          </a>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        {carga.tipo === 'com_troca' && carga.troca && (
                          <div style={styles.trocaBanner}>
                            ...
                            <p style={{ margin: 0, fontSize: '14px', fontWeight: 700, color: '#FFF' }}>{carga.troca.cliente}</p>
                            <p style={{ margin: 0, fontSize: '12px', color: '#FFD700', fontWeight: 500 }}>{carga.troca.cidade}</p>
                          </div>
                        )}

                        <div style={styles.vehicleDetails}>
                          <div style={styles.detailRow}>
                            <span style={{ color: '#888', fontWeight: 600 }}>Placa/Carreta:</span>
                            <span style={{ color: '#FFF', fontWeight: 800 }}>{carga.placa} / {carga.carreta}</span>
                          </div>
                          <div style={styles.detailRow}>
                            <span style={{ color: '#888', fontWeight: 600 }}>Peso:</span>
                            <span style={{ color: '#FFD700', fontWeight: 800 }}>{carga.peso}</span>
                          </div>
                          {carga.pvs && carga.pvs.length > 0 && (
                            <div style={{...styles.detailRow, flexDirection: 'column', gap: '4px', marginTop: '8px' }}>
                              <span style={{ color: '#888', fontWeight: 600 }}>PVs:</span>
                              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                {carga.pvs.map((pv, idx) => (
                                  <span key={idx} style={{ backgroundColor: '#1A1A1A', padding: '2px 8px', borderRadius: '6px', fontSize: '11px', border: '1px solid #333', color: '#AAA' }}>{pv}</span>
                                ))}
                              </div>
                            </div>
                          )}
                    ...
                    {carga.obs && (
                      <div style={{ backgroundColor: '#FFD70020', padding: '12px 16px', borderRadius: '12px', marginBottom: '20px', borderLeft: '4px solid #FFD700' }}>
                        <p style={{ margin: 0, fontSize: '13px', color: '#FFD700', fontWeight: 500 }}>
                    ...
                    <div style={styles.footer}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                          style={styles.actionBtn('#1A1A1A', '#AAA')}
                          onClick={() => { setEditandoCarga(carga); setShowEditModal(true); }}
                        >
                          <Edit3 size={14} /> Editar
                        </button>
                        <button
                          style={styles.actionBtn('#EF444420', '#EF4444')}
                          onClick={async () => {
                      ...
                      {carga.status === 'programada' && (
                        <button
                          style={styles.actionBtn('#22C55E20', '#22C55E')}
                          onClick={async () => {
                            const cargaRef = doc(db, "motoristas", carga.motoristaId!, "cargas", carga.id!);
          ...
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()} id="modal-report">
            <div style={styles.modalHeader} className="no-print">
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#FFF' }}>Relatório Comercial</h2>
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <button style={{...styles.actionBtn('#FFD700', '#000'), padding: '10px 20px' }} onClick={() => window.print()}>
                  <Printer size={18} /> Imprimir PDF
                </button>
                <button style={{...styles.actionBtn('#1A1A', '#AAA'), padding: '10px 14px' }} onClick={() => setShowModal(false)}>
                  <X size={22} />
                </button>
              </div>
            </div>
            ...
            <div style={styles.modalBody} className="report-container">
              <div className="report-header">
                <h1 style={{ margin: 0, color: '#FFF' }}>Logística TG - Relatório de Operações</h1>
              ...
              <div className="stat-grid">
                ...
                <div className="stat-box">
                  ...
                  <p style={{ fontSize: '14px', fontWeight: 700, margin: '4px 0', color: '#FFF' }}>{stats.clientesTop[0]?.[0] || '---'}</p>
                </div>
                <div className="stat-box">
                  <p style={styles.statLabel}>Status Operacional</p>
                  <p style={{ fontSize: '14px', fontWeight: 700, margin: '4px 0', color: '#22C55E' }}>Ativo</p>
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', marginBottom: '32px' }} className="no-print">
                <div>
                  <h3 style={{ fontSize: '16px', fontWeight: 800, marginBottom: '16px', color: '#FFF' }}>Top 10 Clientes (Volume)</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {stats.clientesTop.map(([cliente, qtd]) => (
                      <div key={cliente} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', backgroundColor: '#1A1A', borderRadius: '8px', border: '1px solid #333' }}>
                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#FFF' }}>{cliente}</span>
                        <span style={{ fontSize: '13px', fontWeight: 800, color: '#FFD700' }}>{qtd} viagens</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div>
                  <h3 style={{ fontSize: '16px', fontWeight: 800, marginBottom: '16px', color: '#FFF' }}>Top Motoristas</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {stats.motoristasTop.map(([mot, qtd]) => (
                      <div key={mot} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', backgroundColor: '#1A', borderRadius: '8px', border: '1px solid #333' }}>
                        <span style={{ fontSize: '13px', fontWeight: 600, color: '#FFF' }}>{mot}</span>
                        <span style={{ fontSize: '13px', fontWeight: 800, color: '#22C55E' }}>{qtd} viagens</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <h3 style={{ fontSize: '16px', fontWeight: 800, marginBottom: '12px', color: '#FFF' }}>Listagem Detalhada de Cargas</h3>
              <table>
                <thead>
                  <tr>
                    <th>DT</th>
                ...
                <tbody>
                  {cargasFiltradas.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>{c.peso}</td>
                      <td style={{ color: c.status === 'finalizada'? '#22C55E' : '#FFD700', fontWeight: 700 }}>
      ...
      {showEditModal && editandoCarga && (
        <div style={styles.modalOverlay} onClick={() => { setShowEditModal(false); setEditandoCarga(null); }}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={{ background: 'linear-gradient(135deg, #FFD700, #FFA500)', padding: '10px', borderRadius: '14px' }}><Edit3 size={22} color="#000" /></div><h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#FFF' }}>Editar Carga</h2></div><button style={{...styles.actionBtn('#1A', '#AAA'), padding: '10px 14px' }} onClick={() => { setShowEditModal(false); setEditandoCarga(null); }}><X size={22} /></button></div>
